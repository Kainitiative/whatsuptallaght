...
## 8. AI Editorial Rules & Constraints

These are the exact rules baked into the AI prompts. They govern what the AI is and is not allowed to do when processing a submission into an article. Changing these rules means changing the prompts in `artifacts/api-server/src/lib/ai-pipeline.ts`.

---

### 8.1 Voice & Tone

> *"Tallaght Community writes like a local person sharing real updates — clear, simple, and grounded. It should feel like something you'd read on Facebook or hear from a neighbour."*

- Third person, plain language
- No clickbait, no superlatives, no formal corporate phrasing
- Avoid words like: *distressing*, *highlighted*, *ensuring*, *committed to*, *underpins*
- Warm and community-facing — not a tabloid, not a press release

---

### 8.2 Strict Content Rules

**Rule 1 — Facts only. No invention.**
The AI may only use facts explicitly stated in the submission.
- Must not add background context, inferred details, or embellishments
- Must not write about what an organisation "is known for" or "is committed to" unless the submission says so
- Must not add quotes or opinions that weren't in the submission
- May use simple connecting language to make sentences flow, but must not introduce new facts
- If the submission expresses uncertainty ("I think", "not sure"), the article must reflect that uncertainty — it cannot be presented as confirmed fact

**Rule 2 — Length must match the source.**
- A one-sentence submission → 2–4 sentence article. No padding.
- A detailed submission → up to 300 words
- Hard maximum: 400 words. Never exceeded.
- No filler paragraphs of background context

**Rule 3 — No emotional interpretation.**
- Do not add emotional commentary unless it is explicitly stated in the submission
- The AI reports; it does not editoralise

**Rule 4 — Source attribution.**
- Credit the contributor as *"a local resident"*, *"a community member"*, or by name if one was given in the submission
- Do not invent unnamed sources ("a community member said...") unless those words are literally in the submission

**Rule 5 — Output format.**
- Article body only — no headline, no byline, no category label
- Headline is generated separately in Stage 5 (information extraction)

---

### 8.3 Source Priority (Text vs Image)

When a submission contains both a text message and an image, the rules are explicit:

| Scenario | Source priority |
|---|---|
| Text message + image | **Text leads.** Image provides visual context only — it must not override or redirect the story |
| Text message only | Text is the story |
| Image only (no text) | Image description leads as the only source of information |
| Audio (voice note) | Whisper transcript treated identically to a text message |

The combined text sent to the article writer is tagged clearly:
- `[COMMUNITY MEMBER'S MESSAGE — primary story source]` for text/audio
- `[ATTACHED PHOTO — supporting visual context only, do not make this the story angle]` for images when text is present
- `[ATTACHED PHOTO — only source of information for this submission]` when image-only

---

### 8.4 Image Analysis Rules (Stage 3)

When analysing an attached photo, the vision model is given these constraints:

- Copy dates, times, prices, names, and locations **exactly** as they appear in the image — no paraphrasing or correction
- *"If a date shows 'Apr 18' write 'April 18'. If a time shows '5 pm', write '5 pm'."*
- Report only what is explicitly visible — no interpretation or inferred context
- If any text in the image is unclear or partially visible, say so rather than guessing

---

### 8.5 Confidence Score Formula

The confidence score determines whether an article auto-publishes, goes to the Review Queue, or is rejected. It is calculated from three signals:

```
confidence = (completenessScore × 0.5)
           + (toneConfidence × 0.3)
           + (min(wordCount / 50, 1) × 0.2)
```

| Component | Weight | Source |
|---|---|---|
| `completenessScore` | 50% | Stage 5 — how complete and publishable is the information (0–1) |
| `toneConfidence` | 30% | Stage 4 — how confident the AI is in its category/tone classification (0–1) |
| Word count signal | 20% | Normalised: `min(wordCount / 50, 1)` — a 50+ word submission scores full marks here |

**Routing thresholds:**

| Score | Outcome |
|---|---|
| `≥ 0.75` | Auto-published. Contributor notified with article link. (Threshold configurable in Settings) |
| `0.40 – 0.74` | Held in Review Queue for admin decision |
| `< 0.40` | Rejected. Contributor sent a message asking for more detail |

---

### 8.6 Headline Rules (Stage 5)

The headline is generated by the information extraction stage, not the article writer:

- Maximum **12 words**
- Must be **factual** — no clickbait phrasing
- No exclamation marks
- Should describe the event or situation, not tease it

---

### 8.7 Few-Shot Golden Examples

The article writer receives 2–3 "golden examples" alongside every prompt — real submissions paired with the ideal published article for that category. These teach the AI the preferred style without relying solely on written rules.

- Examples are stored in the `golden_examples` table, managed via the admin dashboard (Golden Examples page)
- Up to 3 examples are loaded per article, matched to the detected category
- The AI is told: *"Match their tone, structure, and level of simplicity. Write in a similar way, but do NOT copy phrases or sentences directly."*
- More examples → more consistent style. Fewer examples → more variability. The admin should aim for at least 2 per category.

---

### 8.8 What the AI Is Never Allowed to Do

Summary of absolute prohibitions regardless of input:

| Prohibited | Reason |
|---|---|
| Invent facts, quotes, or sources | Accuracy and legal risk |
| Write more than 400 words | Prevents padding |
| Use clickbait headlines | Editorial standards |
| Present uncertain facts as confirmed | Accuracy |
| Let an image override a text story | Source integrity |
| Copy phrases from golden examples verbatim | Originality |
| Include a headline or byline in the output | Format integrity — handled separately |
...
### 14.1 AI-Generated Header Images (DALL·E 3)
...
### 14.2 Community Reaction Pieces (WhatsApp Hashtag System)
...
### 14.4 Newsletter System (WhatsApp + Email)
...
### 14.5 Social Media Comments → AI Opinion Pieces
...
### 14.6 Per-Article AI Cost Display
...
### 14.7 Contributor Reputation Tiers
...
### 14.8 Duplicate Detection (Multi-Contributor Articles)
...
### 14.9 Events Calendar & Directory
...
### 14.10 Contributor Pre-Publish Review (WhatsApp Approval Loop)
...
### 14.11 Analytics Integrations (Google + Meta)
...
### 14.13 WhatsApp Message Templates (Outbound beyond 24-Hour Window)
