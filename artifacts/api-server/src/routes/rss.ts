...
router.post("/rss/feeds", async (req, res) => {
  const { name, url, checkIntervalMinutes, filterMode, feedType, keywordFilters } = req.body;
  if (!name || !url) {
  ...
  try {
    const [feed] = await db
      ...
      .values({
        name,
        url,
        checkIntervalMinutes: checkIntervalMinutes ?? 60,
        filterMode: filterMode ?? null,
        feedType: feedType ?? "rss",
        keywordFilters: Array.isArray(keywordFilters) && keywordFilters.length > 0 ? keywordFilters : null,
      })
      .returning();
...
router.patch("/rss/feeds/:id", async (req, res) => {
  ...
  const { name, url, isActive, checkIntervalMinutes, filterMode, feedType, keywordFilters } = req.body;
  ...
  if (feedType !== undefined) updates.feedType = feedType ?? "rss";
  if (keywordFilters !== undefined) updates.keywordFilters = Array.isArray(keywordFilters) && keywordFilters.length > 0 ? keywordFilters : null;
